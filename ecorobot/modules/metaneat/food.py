...
class Food(Module):
    ...
    def __init__(self, hazard_locs=[], direction=0, num_hazards=0, radius=5, name="food", idx=0, target=True, loc=[0, 0, 10.0], loc_type="random", size=1, max_distance=5, min_distance=2, add_joints=True):
        self.has_pos = True
        ...
        self.direction = direction-1 # this is only used to choose the angle in direction
        ...
        if len(self.hazard_locs):
            #self.direction = jax.random.choice([0, 1, 2, 3])
    ...
    def reset(self, rng):
        if self.loc_type == "random":
            ...
            hazard_index = jnp.array([0, 4, 20, 24 ])[self.direction].astype(int)
            neighbors = jnp.array([5, 9,15, 19])
            neighbor = neighbors[hazard_index]
            #hazard_index = jax.random.randint(rng, (1,), 12, 13)
