...
class Fish(PipelineEnv):
  ...
  def __init__(
    ...
    self.xml_file = 'envs/ecorobot/ecorobot/robots/assets/fish.xml'
    self.info_size = 3  # x,y location of robot
    self.init_loc = jnp.array([0.0, 0.0, 0.15])
    self.torso_radius = 0.15

    sys = mjcf.load(self.xml_file)
  ...
  def convert_action(self, action, rng):
      rng, rng1, rng2 = jax.random.split(rng, 3)
      # TODO: how am I sure about the first action corresponding to rotation and the second to force? (take a look at mjcf.py)
      # sample rotation in gaussian distribution
      """
      rotation_mean = action.at[0].get()
      rotation_sigma = action.at[1].get()
      rotation = jax.random.normal(rng1, shape=(1,), dtype=jp.float32) * rotation_sigma + rotation_mean

      force_mean = action.at[2].get()
      force_sigma = action.at[3].get()
      force = jax.random.normal(rng2, shape=(1,), dtype=jp.float32) * force_sigma + force_mean

      """
      rotation = action.at[0].get()
      force = action.at[1].get()
      action = jp.array([rotation, jp.cos(rotation)*force, jp.sin(rotation)*force])
      return jp.squeeze(action), rng
  ...
  def step(self, state: State, action: jp.ndarray) -> State:
    ...
    action, state.info['rng'] = self.convert_action(action, state.info['rng'])
  ...
  def _get_obs(self, pipeline_state: base.State) -> jp.ndarray:
    ...
    robot_pos = qpos[:self.info_size][:2]
    robot_vel = qvel[:self.info_size][:2]

    if self._exclude_current_positions_from_observation:
      robot_info = [robot_pos] + [robot_vel]
    else:
      robot_info = [robot_vel]

    return jp.concatenate(robot_info)