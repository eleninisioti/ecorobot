...
class HazardousPointNavigation(EcorobotEnv):
    ...
    def __init__(self,  genidx=0, robot_type="discrete_fish",  project_dir="projects/metaneat_benchmarking/metaneat_djo/hazardous",num_rangefinders=12,episode_length=0, reset_task=0,rollout_length=0, **kwargs):
    ...
    def step(self, state, action):
        ...
        new_info = state.info
        new_info["cost"] = hazard_penalty


        state = state.replace(obs=obs, reward=reward, pipeline_state=pipeline_state, info=new_info, done=0.0)
    ...
    def _get_obs(self, state: State) -> jnp.ndarray:
        ...
        return jnp.concatenate( [qpos] + [qvel] + range_obs + [to_food[:2]] )
