...
def four_goals(seed: jax.random.PRNGKey):
    length = 1

    #angle = jp.deg2rad([45, 135, 225, 315][seed])
    angles = jp.deg2rad(jp.array([45, 135, 225, 315]))
    angle = jax.lax.switch(seed, [lambda: angles[0],
                             lambda: angles[1],
                             lambda: angles[2],
                             lambda: angles[3]])
...
class HazardousPointEnvRandGoal(Env):
    def __init__(self, goalidx=0, genidx=0, predefined_goals=False, backend=None, **kwargs):  # Can set goal to test adaptation.
        super().__init__()
    ...
    def render(self, agent_locs, save_dir):


        fig, ax = plt.subplots()
        goal_loc = agent_locs[0][-2:]
        # plot food
        ax.scatter(x=goal_loc[0], y=goal_loc[1], color="green")
        ...
        for agent_loc in agent_locs:
            # plot agent
            ax.scatter(x=agent_loc[0], y=agent_loc[1], color="orange")

            # plot rangefinder points
            #for lidar in self.lidar_points:
            #    ax.scatter(x=agent_loc[0] + lidar[0], y=agent_loc[1]+lidar[1], color="orange")
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        seed = jax.random.randint(rng, shape=(), minval=0,maxval=4).astype(jp.int32)
        _goal = four_goals(seed)
        ...
        pipeline_state = base.State(
            q=None, #jp.zeros(1),
            qd=None, #jp.zeros(1),
            x=base.Transform.create(pos=jp.concat([jp.zeros(2),_goal], axis=0)),
            xd=base.Motion.create(vel=jp.zeros(1)),
            contact=None
        )
        obs = jp.zeros(14)
        return State(pipeline_state, obs, reward, done, metrics, info={'goal': _goal, 'cost': 0.0})
    ...
    def get_hazard_penalty(self, robot_pos):
        ...
        penalty = jp.where(jp.min(distances) < thres, 1.0, 0.0)
        #penalty = 0.0
        return penalty
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        agent_pos  = state.pipeline_state.x.pos[:2]
        new_agent_position = agent_pos  + action
        #delta_position = new_position - self._goal
        _goal = state.pipeline_state.x.pos[-2:]
        new_position = jp.concatenate([new_agent_position,_goal], axis=0)

        distance = jp.sqrt(jp.sum((new_agent_position - _goal) ** 2))
        ...
        penalty = self.get_hazard_penalty(new_agent_position)
        #reward = reward-penalty

        next_observation = new_agent_position

        # add lidar info to observation
        lidar_obs = jax.vmap(self.get_lidar_obs, in_axes=(None, 0))(new_agent_position, self.lidar_points)
        next_observation = jp.concatenate([next_observation, lidar_obs], axis=0)

        state.metrics.update(
            x_position=new_position[0],
            y_position=new_position[1],
            distance_from_origin=math.safe_norm(new_agent_position),
        ...
        new_info = state.info
        new_info["cost"] = penalty
        return State(new_pipeline_state, next_observation, reward=reward, done=done, metrics=state.metrics, info=new_info)
