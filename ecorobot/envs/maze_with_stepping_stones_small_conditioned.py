...
class SteppingStoneMazeSmall(EcorobotEnv):
    ...
    def reset(self, key, env_params=None):
        ...
        obs = self._get_obs(new_state.pipeline_state, 0)
    ...
    def step_debug(self, state, action, debug_robot_pos):
        ...
        obs = self._get_obs(pipeline_state, state.info["n_stones"])
    ...
    def _get_obs(self, pipeline_state: State, current_stone) -> jnp.ndarray:
        ...
        goal_pos = jnp.array(self.stone_locs)[current_stone]
        return jnp.concatenate([qpos] + [qvel] + [rangefinder_sensor_info] + [goal_pos])
